...
Источник сценария: /mnt/d/IWE/CLAUDE.md → Протокол Day-Close

## Контекст

- **WeekPlan:** /mnt/d/IWE/DS-strategy/current/WeekPlan W*.md (последний по дате)
- **MEMORY:** ~/.claude/projects/-mnt-d-IWE/memory/MEMORY.md
- **Exocortex backup:** /mnt/d/IWE/DS-strategy/exocortex/
...
```bash
# Для КАЖДОГО репо в /mnt/d/IWE/:
git -C /mnt/d/IWE/<repo> log --since="today 00:00" --oneline --no-merges
```

- Пройди по ВСЕМ репозиториям в `/mnt/d/IWE/`
...
Скопируй актуальные файлы в `/mnt/d/IWE/DS-strategy/exocortex/`:

```bash
# Корневой CLAUDE.md
cp /mnt/d/IWE/CLAUDE.md /mnt/d/IWE/DS-strategy/exocortex/CLAUDE.md

# Memory (Слой 3)
cp ~/.claude/projects/-mnt-d-IWE/memory/MEMORY.md /mnt/d/IWE/DS-strategy/exocortex/MEMORY.md
cp ~/.claude/projects/-mnt-d-IWE/memory/*.md /mnt/d/IWE/DS-strategy/exocortex/
```
