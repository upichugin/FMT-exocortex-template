...
2. Заменяет 7 плейсхолдеров (`upichugin`, `/mnt/d/IWE` и др.)
...
1. Замените `upichugin` на ваш GitHub username во всех файлах
2. Замените `/mnt/d/IWE` на путь к рабочей директории (напр. `~/Github`)
3. Замените `/home/user` на домашнюю директорию (значение `$HOME`)
4. Замените `-mnt-d-IWE` на путь через дефисы (напр. для `~/Github` → `-Users-yourname-Github`)
